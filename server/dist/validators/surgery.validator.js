...
const dateString = zod_1.z
    .string()
    .min(1, 'Operation date is required')
    .refine((value) => !Number.isNaN(Date.parse(value)), {
    message: 'Invalid date',
});
const timeString = zod_1.z
    .string()
    .min(1, 'Operation time is required')
...
const operationBodySchema = zod_1.z.object({
    ...
    medicalTeam: zod_1.z.object({
        doctorIds: zod_1.z.array(zod_1.z.string().uuid()).optional(),
        nurseIds: zod_1.z.array(zod_1.z.string().uuid()).optional(),
        primarySurgeonId: zod_1.z.string().uuid().optional(),
        assistantSurgeonId: zod_1.z.string().uuid().optional(),
        anesthesiologistId: zod_1.z.string().uuid().optional(),
        assistantAnesthesiaId: zod_1.z.string().uuid().optional(),
        nurse: zod_1.z.string().optional(),
        notes: zod_1.z.string().optional(),
    }).optional(),
...
exports.operationQuerySchema = zod_1.z.object({
    ...
    sortBy: zod_1.z.enum(['operationDate', 'createdAt', 'name', 'duration']).default('operationDate'),

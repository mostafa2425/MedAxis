...
exports.swaggerSpec = {
    ...
    info: {
        title: 'MedAxis API',
        version: '1.0.0',
        description: 'MedAxis Medical Management API. Authenticated endpoints require a JWT Bearer token obtained from `/api/auth/login` or `/api/auth/register`. Specialty create/update/delete require the `admin` role.',
    },
    servers: [
        {
    ...
    components: {
        ...
        schemas: {
            ...
            FileType: {
                type: 'string',
                description: 'Stored enum values. Upload aliases: `BEFORE_OPERATION` → `BEFORE_IMAGE`, `AFTER_OPERATION` → `AFTER_IMAGE`.',
                enum: [
                    'BEFORE_IMAGE',
                    'BEFORE_XRAY',
                    'BEFORE_MRI',
    ...
    paths: {
        ...
        '/api/specialties': {
            get: {
                ...
                description: 'Global specialty catalog. Omit filters to return all active specialties (top-level and areas). Use `parentId`/`parentIds` to return children of selected specialties. Use `mine=true` (authenticated) to return only areas belonging to the current doctor\'s selected specialties.',
                security: [],
                parameters: [
                    {
                        name: 'parentId',
                    ...
                    },
                    {
                        ...
                        in: 'query',
                        schema: { type: 'boolean' },
                        description: 'When true, requires a Bearer token and restricts parent IDs to the current doctor\'s selected specialties.',
                    },
                    {
                        name: 'rootsOnly',
                        in: 'query',
        ...
        '/api/operation-catalog': {
            get: {
                ...
                description: 'Returns common catalog operations for the doctor\'s top-level specialties (prioritizing selected areas of expertise) plus that doctor\'s custom operations. Unrelated specialties are excluded.',
                security: bearerAuth,
                responses: {
                    '200': {
        ...
        '/api/operations/{id}/files': {
            post: {
                ...
                description: 'Multipart upload. Send `file` and/or `files` (up to 20). `fileType` is required for the intended slot; aliases `BEFORE_OPERATION` and `AFTER_OPERATION` map to `BEFORE_IMAGE` / `AFTER_IMAGE`. Max file size 50MB. Storage is the local `uploads` directory (or `UPLOAD_DIR`).',
                security: bearerAuth,
                parameters: [{ $ref: '#/components/parameters/IdParam' }],
                requestBody: {
                    required: true,
                        ...
                        'multipart/form-data': {
                            schema: {
                                ...
                                properties: {
                                    ...
                                    fileType: {
                                        type: 'string',
                                        description: 'Prisma FileType or alias BEFORE_OPERATION / AFTER_OPERATION',
                                    },
                                },
                            },
                        },
        ...
        '/api/export/operations': {
            get: {
                ...
                description: 'Returns JSON by default. Set `format=csv` to download a CSV attachment.',
                security: bearerAuth,
                parameters: [
                    {
                        name: 'format',

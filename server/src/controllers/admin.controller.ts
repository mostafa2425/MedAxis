...
export const adminController = {
  ...
  async updateUser(req: AuthenticatedRequest, res: Response, next: NextFunction) { try { const data = await adminService.updateUser(req.params.id, { role: q(req.body?.role), ...(req.body?.isActive !== undefined ? { isActive: bool(req.body.isActive) } : {}) }); return sendSuccess(res, data, 'User updated'); } catch (error) { return next(error); } },

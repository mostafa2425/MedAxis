...
export class OperationRepository {
  ...
  async replaceTeam(operationId: string, teamMembers: Array<{ doctorId?: string | null; nurseId?: string | null; sortOrder: number }>) {
    await prisma.$transaction([prisma.operationTeamMember.deleteMany({ where: { operationId } }), prisma.operationTeamMember.createMany({ data: teamMembers.map((m) => ({ operationId, doctorId: m.doctorId ?? null, nurseId: m.nurseId ?? null, sortOrder: m.sortOrder })) })]);
  }

  // Backwards-compatible aliases used by OperationService.
  ...
  async upsertCost(operationId: string, data: CostData) {
    return prisma.operationCost.upsert({ where: { operationId }, update: data as any, create: { operationId, totalCost: data.totalCost, paidAmount: data.paidAmount ?? 0, remainingAmount: data.remainingAmount ?? (data.totalCost - (data.paidAmount ?? 0)), hospitalCost: data.hospitalCost ?? 0, nursingCost: data.nursingCost ?? 0, assistantDoctorsCost: data.assistantDoctorsCost ?? 0, equipmentCost: data.equipmentCost ?? 0, otherCost: data.otherCost ?? 0, paymentMethod: data.paymentMethod as any, paymentStatus: data.paymentStatus as any, paymentNotes: data.paymentNotes } });
  }
  ...
  async addFile(operationId: string, data: { fileType: string; fileName: string; filePath: string; fileSize?: number; mimeType?: string; uploadedBy: string }) {
    const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
      INSERT INTO "operation_files" ("id", "operationId", "fileType", "fileName", "filePath", "storagePath", "fileSize", "mimeType", "uploadedBy", "createdAt")
      VALUES (gen_random_uuid(), ${operationId}, ${data.fileType}::"FileType", ${data.fileName}, ${data.filePath}, ${data.filePath}, ${data.fileSize ?? null}, ${data.mimeType ?? null}, ${data.uploadedBy}, now())
      RETURNING *
    `);
    return rows[0];
  }
  ...
  async countByStatus(createdBy: string) {
    ...
    return result.reduce((acc, item) => { acc[item.status] = item._count.status; return acc; }, {} as Record<string, number>);
  }
  ...
  async countBySpecialty(createdBy: string) {
    ...
    const specialties = await prisma.specialty.findMany({ where: { id: { in: result.map((r) => r.specialtyId!) } }, select: { id: true, name: true } });
  ...
  async monthlyTrends(createdBy: string, months = 12) {
    const start = new Date(); start.setMonth(start.getMonth() - (months - 1), 1); start.setHours(0, 0, 0, 0);
    ...
    for (let i = 0; i < months; i++) { const date = new Date(start); date.setMonth(start.getMonth() + i); const month = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; const monthOps = operations.filter((op) => `${op.operationDate.getFullYear()}-${String(op.operationDate.getMonth() + 1).padStart(2, '0')}` === month); result.push({ month, total: monthOps.length, completed: monthOps.filter((op) => op.status === 'COMPLETED').length }); }
    return result;
  }

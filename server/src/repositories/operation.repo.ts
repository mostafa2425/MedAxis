...
export class OperationRepository {
  ...
  async create(data: { name: string; diagnosis?: string | null; hospitalId: string; operationDate: Date; operationTime: string; operationRoom?: string; duration?: number; status?: OperationStatus; notes?: string; patientId: string; createdBy: string; specialtyId?: string | null; catalogId?: string | null; procedures?: Array<{ catalogId?: string | null; name: string; nameAr?: string | null; specialtyId?: string | null; sortOrder: number }>; teamMembers?: Array<{ doctorId?: string | null; nurseId?: string | null; sortOrder: number }>; medicalTeam?: { primarySurgeonId?: string; assistantSurgeonId?: string; anesthesiologistId?: string; assistantAnesthesiaId?: string; nurse?: string; notes?: string }; cost?: { totalCost: number; paidAmount?: number; remainingAmount?: number; paymentMethod?: string; paymentStatus?: string; paymentNotes?: string } }) {
    ...
    return prisma.operation.create({
      data: {
        ...
        ...(cost ? { cost: { create: { totalCost: cost.totalCost, paidAmount: cost.paidAmount ?? 0, remainingAmount: cost.remainingAmount ?? (cost.totalCost - (cost.paidAmount ?? 0)), paymentMethod: cost.paymentMethod as any, paymentStatus: cost.paymentStatus as any, paymentNotes: cost.paymentNotes } } } : {}),
      },
  ...
  async replaceProcedures(operationId: string, procedures: Array<{ catalogId?: string | null; name: string; nameAr?: string | null; specialtyId?: string | null; sortOrder: number }>) {
    await prisma.$transaction([
      ...
      prisma.operationProcedure.createMany({ data: procedures.map((procedure) => ({ operationId, catalogId: procedure.catalogId ?? null, name: procedure.name, nameAr: procedure.nameAr ?? null, specialtyId: procedure.specialtyId ?? null, sortOrder: procedure.sortOrder })) }),
    ]);
  }
  ...
  async replaceTeamMembers(operationId: string, members: Array<{ doctorId?: string | null; nurseId?: string | null; sortOrder: number }>) {
    await prisma.$transaction([
      ...
      ...(members.length ? [prisma.operationTeamMember.createMany({ data: members.map((member) => ({ operationId, doctorId: member.doctorId ?? null, nurseId: member.nurseId ?? null, sortOrder: member.sortOrder })) })] : []),
    ]);
  }
  ...
  async upsertCost(operationId: string, data: { totalCost: number; paidAmount?: number; remainingAmount?: number; paymentMethod?: string; paymentStatus?: string; paymentNotes?: string }) {
    return prisma.operationCost.upsert({
      where: { operationId },
      update: data as any,
      create: { operationId, totalCost: data.totalCost, paidAmount: data.paidAmount ?? 0, remainingAmount: data.remainingAmount ?? (data.totalCost - (data.paidAmount ?? 0)), paymentMethod: data.paymentMethod as any, paymentStatus: data.paymentStatus as any, paymentNotes: data.paymentNotes },
    });
  }
  ...
  async addTimeline(operationId: string, data: { action: string; description?: string; userId: string }) {
    return prisma.operationTimeline.create({ data: { operationId, ...data } as any });
  }

  async getTimeline(operationId: string) {
    return prisma.operationTimeline.findMany({ where: { operationId }, include: { user: { select: { id: true, name: true } } }, orderBy: { createdAt: 'desc' } });
  }

  async getRecent(createdBy: string, limit = 5) {
    return prisma.operation.findMany({ where: { createdBy }, take: limit, include: { patient: true, hospital: true, specialty: true, cost: true }, orderBy: { createdAt: 'desc' } });
  }
  ...
  async getMonthlyTrends(createdBy: string, months = 12) {
    ...
    startDate.setDate(1);
    const operations = await prisma.operation.findMany({ where: { createdBy, operationDate: { gte: startDate } }, select: { operationDate: true, status: true }, orderBy: { operationDate: 'asc' } });
  ...
  async getTotalRevenue(createdBy: string) {
    const costs = await prisma.operationCost.findMany({ where: { operation: { createdBy } }, select: { totalCost: true, paidAmount: true, remainingAmount: true } });
    return {

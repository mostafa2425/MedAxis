...
function toSpecialtyRef(link: { specialty: { id: string; name: string; nameAr: string | null } }) { return { id: link.specialty.id, name: link.specialty.name, nameAr: link.specialty.nameAr }; }
...
class DoctorService {
  async getAll(params: { page: number; limit: number; search?: string; specialtyId?: string; userId: string }) {
    const { data, total } = await doctorRepo.findAll(params);
    return { data: data.map(toDoctorResponse), total };
  }
  async getActive(userId: string) { return (await doctorRepo.findActive(userId)).map(toDoctorResponse); }
  async getById(id: string, userId: string) {
    const doctor = await doctorRepo.findById(id, userId);
    if (!doctor) throw new NotFoundError('Doctor');
    return toDoctorResponse(doctor);
  }
  async assertAccessible(id: string, userId: string) { await this.getById(id, userId); }
  async create(input: CreateDoctorInput, userId: string) {
    ...
    if (duplicate) return toDoctorResponse(duplicate);
    const { validSpecialtyIds, validSubspecialtyIds } = await resolveSpecialtyLinks(specialtyIds, subspecialtyIds);
    return toDoctorResponse(await doctorRepo.create({ name: data.name, phone: data.phone ?? null, email: data.email ?? null, createdBy: userId }, validSpecialtyIds, validSubspecialtyIds));
  }
  async update(id: string, userId: string, input: UpdateDoctorInput) {
    ...
    const { specialtyIds, subspecialtyIds, ...data } = input;
    if (Object.keys(data).length > 0) await doctorRepo.update(id, data);
  ...
  async delete(id: string, userId: string) {
    const owned = await doctorRepo.findOwned(id, userId);
    if (!owned) throw new NotFoundError('Doctor');
    if (owned.userId === userId) throw new ForbiddenError('Cannot remove your own doctor profile');

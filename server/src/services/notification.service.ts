...
export class NotificationService {
  ...
  async markRead(userId: string, notificationId: string) {
    await prisma.$executeRawUnsafe(`update public.smart_notifications set read_at = now() where id = $1::uuid and user_id = $2::uuid`, notificationId, userId);
  }
  ...
  async createTest(userId: string) {
    ...
    const inserted = await prisma.$queryRawUnsafe<Array<{ id: string }>>(
      `insert into public.smart_notifications (user_id, kind, title, message, priority, scheduled_for)
       values ($1::uuid, 'DAILY_BRIEF', $2, $3, 'normal', $4) returning id`, userId, title, message, scheduledFor,
    );
    const notificationId = inserted[0]?.id;
    if (!notificationId) throw new Error('Failed to create test notification');
    const push = await pushService.sendToUser(userId, { title, body: message, url: '/assistant', tag: 'MEDAXIS_TEST_NOTIFICATION', kind: 'TEST_NOTIFICATION', notificationId });
  ...
  private async createBrief(userId: string, kind: NotificationKind, scheduledFor: Date, start: Date, end: Date) {
    ...
    const prefix = kind === 'DAILY_BRIEF' ? 'Yesterday' : 'Last week';
    const s = brief.summary;
    const message = `${prefix}: ${s.operations} operations (${Object.entries(s.operationStatus).map(([k, v]) => `${v} ${k.toLowerCase()}`).join(', ') || 'none'}) · ${s.newPatients} new patients · ${s.followUps} follow-ups · ${s.overdueFollowUps} overdue · ${s.attention} items need attention`;
    const priority = s.attention > 0 ? 'important' : 'normal';
    const title = kind === 'DAILY_BRIEF' ? 'Your daily summary' : 'Your weekly summary';

    const inserted = await prisma.$queryRawUnsafe<Array<{ id: string }>>(
      `insert into public.smart_notifications (user_id, kind, title, message, priority, scheduled_for)
       values ($1::uuid, $2, $3, $4, $5, $6)
       on conflict (user_id, kind, scheduled_for) do nothing returning id`,
      userId, kind, title, message, priority, scheduledFor,
    );

    if (inserted[0]?.id) {
      await pushService.sendToUser(userId, { title, body: message, url: '/assistant', tag: kind, kind, notificationId: inserted[0].id });
    }
    return brief;
  }

  // Runs at 00:00 Egypt time and reports the previous calendar day.
  async processDaily() {
    const { start, end } = egyptDayBounds(-1);
    const scheduledFor = new Date(end.getTime());
  ...
  // Runs at 00:00 Egypt time on Sunday and reports the previous 7 days.
  async processWeekly() {

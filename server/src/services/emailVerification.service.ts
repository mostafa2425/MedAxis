...
function smtpCommand(socket: tls.TLSSocket, command: string, accepted: number[] = [250]) {
  return new Promise<void>((resolve, reject) => {
    ...
    const onData = (chunk: Buffer) => {
      ...
      if (accepted.includes(code)) resolve(); else reject(new Error(`SMTP ${code}: ${last}`));
    };
...
export class EmailVerificationService {
  async issue(userId: string, email: string, name: string, enforceCooldown = true) {
    ...
    await prisma.$executeRaw`INSERT INTO "email_verification_tokens" ("id", "userId", "tokenHash", "expiresAt", "createdAt") VALUES (${randomBytes(16).toString('hex')}, ${userId}, ${tokenHash}, ${expiresAt}, NOW())`;
    try { await sendEmail(email, name, verificationUrl(getConfig().appUrl, token)); }
    catch (error) { await prisma.$executeRaw`DELETE FROM "email_verification_tokens" WHERE "tokenHash" = ${tokenHash}`; throw error; }
  }

  async verify(token: string) {
    if (!token || token.length < 32) throw new AppError('Invalid verification link', 400);
    const tokenHash = hashToken(token);
    const rows = await prisma.$queryRaw<Array<{ id: string; userId: string; expiresAt: Date }>>`SELECT "id", "userId", "expiresAt" FROM "email_verification_tokens" WHERE "tokenHash" = ${tokenHash} LIMIT 1`;
    const record = rows[0];
    if (!record) throw new AppError('This verification link is invalid or has already been used.', 400);
    if (new Date(record.expiresAt).getTime() <= Date.now()) { await prisma.$executeRaw`DELETE FROM "email_verification_tokens" WHERE "id" = ${record.id}`; throw new AppError('This verification link has expired. Please request a new one.', 400); }
  ...
  async resend(email: string) {
    const rows = await prisma.$queryRaw<Array<{ id: string; email: string; name: string; emailVerifiedAt: Date | null; isActive: boolean }>>`SELECT "id", "email", "name", "emailVerifiedAt", "isActive" FROM "users" WHERE LOWER("email") = LOWER(${email}) LIMIT 1`;
    const user = rows[0];

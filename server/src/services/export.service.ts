...
class ExportService {
  ...
  private toJsonCsv(operations: any[]) {
    ...
    const headers = [
      'Operation Name', 'Diagnosis', 'Date', 'Time', 'Duration (min)', 'Status',
      'Patient', 'Age', 'Gender', 'Mobile',
      'Hospital', 'Specialty',
      'Primary Surgeon', 'Assistant Surgeon', 'Anesthesiologist',
      'Total Cost', 'Paid Amount', 'Remaining', 'Payment Method', 'Payment Status',
      'Notes',
    ];
    ...
    const rows = operations.map((op) => [
      ...
      op.status || '',
      this.escapeCsv(op.patient?.fullName || ''),
      op.patient?.age?.toString() || '',
      op.patient?.gender || '',
      op.patient?.mobile || '',
      this.escapeCsv(op.hospital?.name || ''),
      this.escapeCsv(op.specialty?.name || ''),
      this.escapeCsv(op.medicalTeam?.[0]?.primarySurgeon?.name || ''),
      this.escapeCsv(op.medicalTeam?.[0]?.assistantSurgeon?.name || ''),
      this.escapeCsv(op.medicalTeam?.[0]?.anesthesiologist?.name || ''),
      op.cost?.totalCost?.toString() || '0',
      op.cost?.paidAmount?.toString() || '0',
      op.cost?.remainingAmount?.toString() || '0',
      op.cost?.paymentMethod || '',
      op.cost?.paymentStatus || '',
      this.escapeCsv(op.notes || ''),
    ]);

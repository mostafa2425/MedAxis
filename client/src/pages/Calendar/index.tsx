...
export default function CalendarPage() {
  ...
  const serverFilters = useMemo(
    ...
    [filters.status, filters.hospitalId, filters.specialtyId],
  );
  ...
  const {
    ...
    queryKey: ['calendar-operations', dateFrom, dateTo, serverFilters],
    queryFn: () =>
      fetchCalendarOperations({
        dateFrom,
        dateTo,
        ...serverFilters,
      }),
  ...
  const { data: upcomingOps = [], isLoading: upcomingLoading } = useQuery({
    queryKey: ['calendar-upcoming', upcomingFrom, upcomingTo, serverFilters],
    queryFn: () =>
      fetchCalendarOperations({
  ...
  const operations = useMemo(
    () => applyCalendarFilters(calendarOps, filters),
    [calendarOps, filters],
  );

  const upcomingFiltered = useMemo(() => {
    return applyCalendarFilters(upcomingOps, filters)
      .filter((operation) => isUpcomingStatus(operation.status))
      .filter((operation) => !parseOperationDateTime(operation).isBefore(dayjs().startOf('day')))
  ...
  const summary = useMemo(() => {
    ...
    const weekEnd = dayjs().locale(locale).endOf('week');
    const source = applyCalendarFilters(upcomingOps, filters);

    const countOn = (key: string) =>
      source.filter((operation) => String(operation.operationDate).slice(0, 10) === key).length;

    const weekCount = source.filter((operation) => {
      const date = dayjs(String(operation.operationDate).slice(0, 10));
      return !date.isBefore(weekStart, 'day') && !date.isAfter(weekEnd, 'day');
    }).length;

    const next = upcomingFiltered.find((operation) => !parseOperationDateTime(operation).isBefore(dayjs(), 'minute'));
    let nextLabel = t('calendar.none');
    if (next) {
      const at = parseOperationDateTime(next);
      const minutes = at.diff(dayjs(), 'minute');
      if (minutes <= 0) nextLabel = formatEventTime(next);
      else if (minutes < 60) nextLabel = t('calendar.inMinutes', { count: minutes });
      else nextLabel = t('calendar.inHours', { hours: Math.floor(minutes / 60), minutes: minutes % 60 });
    }

    return {
      todayCount: countOn(todayKey),
      tomorrowCount: countOn(tomorrowKey),
      weekCount,
      nextLabel,
    };
        ...
        <div className="pageHeader">
          <div className="pageHeaderLeft">
            ...
            <div className="pageHeaderText">
              ...
              <p className="pageHint">{t('calendar.subtitle')}</p>
            </div>
          </div>
        </div>
        <Empty
    ...
    <div className="calendar-page page">
      <div className="pageHeader">
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={goCreate}>
          {t('calendar.newOperation')}
        </Button>
      </div>

      <CalendarSummary
        todayCount={summary.todayCount}
        tomorrowCount={summary.tomorrowCount}
        weekCount={summary.weekCount}
        nextOperationLabel={summary.nextLabel}
      ...
      <div className="calendarShell">
        <div className="calendarMain">
          <CalendarToolbar
            ...
            onPrevious={() => setCurrent((value) => shiftPeriod(view, value, -1))}
            onNext={() => setCurrent((value) => shiftPeriod(view, value, 1))}
          ...
          <Spin spinning={isLoading || isFetching}>
            {isLoading ? (
              ...
              <WeekView current={current} operations={operations} onCreate={goCreate} />
            ) : view === 'day' ? (
              <DayView current={current} operations={operations} onCreate={goCreate} />
            ) : view === 'agenda' ? (
              <AgendaView operations={operations} onCreate={goCreate} />
            ) : (
              <MonthView
                current={current}
        ...
        <UpcomingOperations operations={upcomingFiltered} loading={upcomingLoading} />
      </div>

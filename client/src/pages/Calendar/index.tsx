...
export default function CalendarPage() {
  ...
  const summary = useMemo(() => {
    ...
    const countOn = (key: string) => source.filter((operation) => String(operation.operationDate).slice(0, 10) === key).length;
    const weekCount = source.filter((operation) => { const date = dayjs(String(operation.operationDate).slice(0, 10)); return !date.isBefore(weekStart, 'day') && !date.isAfter(weekEnd, 'day'); }).length;
    ...
    if (next) { const at = parseOperationDateTime(next); const minutes = at.diff(dayjs(), 'minute'); if (minutes <= 0) nextLabel = formatEventTime(next); else if (minutes < 60) nextLabel = t('calendar.inMinutes', { count: minutes }); else nextLabel = t('calendar.inHours', { hours: Math.floor(minutes / 60), minutes: minutes % 60 }); }
    return { todayCount: countOn(todayKey), tomorrowCount: countOn(tomorrowKey), weekCount, nextLabel };
  ...
  if (isError) {
    return <div className="calendar-page page"><div className="pageHeader"><div className="pageHeaderLeft"><div className="pageIcon"><CalendarOutlined /></div><div className="pageHeaderText"><h1 className="pageTitle">{t('calendar.title')}</h1><p className="pageHint">{t('calendar.subtitle')}</p></div></div></div><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t('common.operationFailed')}><Button type="primary" onClick={() => refetch()}>{t('common.refresh')}</Button></Empty></div>;
  }
      ...
      <div className="calendarShell">
        <div className="calendarMain">
          ...
          <Spin spinning={isLoading || isFetching}>
            {isLoading ? <Skeleton active paragraph={{ rows: 8 }} /> : view === 'week' ? <WeekView current={current} operations={operations} onCreate={goCreate} /> : view === 'day' ? <DayView current={current} operations={operations} onCreate={goCreate} /> : view === 'agenda' ? <AgendaView operations={operations} onCreate={goCreate} /> : <MonthView current={current} operations={operations} onChange={setCurrent} onShowDay={(value) => { setCurrent(value); setView('day'); }} onCreate={goCreate} />}
          </Spin>
        </div>

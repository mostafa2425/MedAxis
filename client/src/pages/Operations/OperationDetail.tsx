...
function StaffCard({ name, role, subtitle, phone }: { name: string; role: string; subtitle?: string; phone?: string }) {
  return <Card size="small" className="medicalStaffCard" bordered>
    <Flex align="center" gap={12}>
      <Avatar size={44} className="medicalStaffAvatar">{getInitials(name)}</Avatar>
      <Flex vertical gap={2} style={{ minWidth: 0, flex: 1 }}>
        <Typography.Text strong ellipsis>{name}</Typography.Text>
        <Tag className="staffRoleTag">{role}</Tag>
        {subtitle && <Typography.Text type="secondary" ellipsis>{subtitle}</Typography.Text>}
...
function FileTile({ file, onDelete }: { file: OperationFile; onDelete: (id: string) => void }) {
  ...
  return <Card size="small" className="clinicalFileCard" bodyStyle={{ padding: 0 }}>
    <div className="clinicalFilePreview">
      {image && url ? <Image src={url} preview={{ mask: <Flex align="center" gap={6}><EyeOutlined />Preview</Flex> }} /> : <FileTextOutlined className="clinicalFileIcon" />}
    </div>
    <Flex vertical gap={4} className="clinicalFileBody">
      <Typography.Text strong ellipsis title={file.fileName}>{file.fileName}</Typography.Text>
      <Tag className="fileTypeTag">{file.fileType}</Tag>
      <Flex justify="space-between" align="center">
        {url ? <Button type="link" size="small" icon={<DownloadOutlined />} href={url} target="_blank">Open</Button> : <span />}
        <Popconfirm title="Delete this file?" onConfirm={() => onDelete(file.id)}><Button type="text" danger size="small" icon={<DeleteOutlined />} /></Popconfirm>
      </Flex>
    </Flex>
  </Card>;
}
...
function FileGroup({ title, description, files, onDelete, accent }: { title: string; description: string; files: OperationFile[]; onDelete: (id: string) => void; accent: 'before' | 'after' | 'other' }) {
  return <section className={`clinicalFileGroup clinicalFileGroup-${accent}`}>
    <Flex justify="space-between" align="end" gap={12} className="clinicalGroupHeader">
      <div><Typography.Title level={4} style={{ margin: 0 }}>{title}</Typography.Title><Typography.Text type="secondary">{description}</Typography.Text></div>
      <Tag>{files.length}</Tag>
    </Flex>
    {files.length ? <div className="clinicalFileGrid">{files.map((file) => <FileTile key={file.id} file={file} onDelete={onDelete} />)}</div> : <div className="clinicalGroupEmpty"><FileImageOutlined /><Typography.Text type="secondary">No files added yet</Typography.Text></div>}
  </section>;
}
...
export default function OperationDetailPage() {
  ...
  const clinicalFiles = <Flex vertical gap={20}>
    <Card className="clinicalFilesHero">
      <Flex justify="space-between" align="center" gap={12} wrap>
        <div><Typography.Title level={3} style={{ margin: 0 }}>Clinical Files</Typography.Title><Typography.Text type="secondary">Prioritize the case story: before, after, imaging, labs and supporting documents.</Typography.Text></div>
        <Space.Compact><Select value={uploadType} onChange={setUploadType} options={[{ value: 'BEFORE_IMAGE', label: 'Before' }, { value: 'AFTER_IMAGE', label: 'After' }, { value: 'XRAY', label: 'X-Ray' }, { value: 'MRI', label: 'MRI' }, { value: 'CT_SCAN', label: 'CT / Scan' }, { value: 'LAB_RESULT', label: 'Lab Result' }, { value: 'OTHER', label: 'Other' }]} /><Upload showUploadList={false} beforeUpload={upload}><Button type="primary" icon={<UploadOutlined />} loading={uploadMutation.isPending}>Upload</Button></Upload></Space.Compact>
      </Flex>
    </Card>
    <FileGroup title="Before" description="Pre-operative photos and baseline clinical images." files={beforeFiles} onDelete={(fileId) => deleteFileMutation.mutate(fileId)} accent="before" />
    <FileGroup title="After" description="Post-operative photos and outcome documentation." files={afterFiles} onDelete={(fileId) => deleteFileMutation.mutate(fileId)} accent="after" />
    <Flex gap={16} wrap><div style={{ flex: '1 1 360px' }}><FileGroup title="Imaging" description="X-Ray, MRI, CT and ultrasound." files={imagingFiles} onDelete={(fileId) => deleteFileMutation.mutate(fileId)} accent="other" /></div><div style={{ flex: '1 1 360px' }}><FileGroup title="Labs & Analyses" description="Lab results and diagnostic reports." files={labFiles} onDelete={(fileId) => deleteFileMutation.mutate(fileId)} accent="other" /></div></Flex>
    <FileGroup title="Other Documents" description="Consent, reports and other supporting files." files={otherFiles} onDelete={(fileId) => deleteFileMutation.mutate(fileId)} accent="other" />
  </Flex>;

  const overview = <Flex vertical gap={16}>
    <Card className="case-hero-card">
      <Flex justify="space-between" align="flex-start" gap={16} wrap>
        <div className="caseHeroMain"><Typography.Text type="secondary">Surgical Case</Typography.Text><Typography.Title level={2} style={{ margin: '4px 0 10px' }}>{operation.name}</Typography.Title><Space wrap><Tag color={statusColor}>{OPERATION_STATUSES.find((s) => s.value === operation.status)?.label ?? operation.status}</Tag>{operation.specialty && <Tag color="blue">{getSpecialtyLabel(operation.specialty, i18n.language)}</Tag>}</Space></div>
        <Select className="statusSelect" value={operation.status} onChange={(status) => changeStatusMutation.mutate(status)} loading={changeStatusMutation.isPending} options={OPERATION_STATUSES.map((s) => ({ value: s.value, label: s.label }))} />
      </Flex>
      <div className="caseMetaGrid">
        <div className="caseMetaItem"><Typography.Text type="secondary">Patient</Typography.Text><Typography.Text strong>{operation.patient?.fullName ?? '—'}</Typography.Text></div>
        <div className="caseMetaItem"><Typography.Text type="secondary">Hospital</Typography.Text><Typography.Text strong>{operation.hospital?.name ?? '—'}</Typography.Text></div>
        <div className="caseMetaItem caseMetaHighlight"><Typography.Text type="secondary"><CalendarOutlined /> Date & Time</Typography.Text><Tag color="blue">{formatOperationDate(operation.operationDate)} · {formatTime(operation.operationTime)}</Tag></div>
    ...
    <Flex gap={16} wrap>
      ...
      <Card title="Procedure" className="detailInfoCard" style={{ flex: '1 1 420px' }}>
        <Flex vertical gap={10}>{procedures.map((procedure, index) => <div key={procedure.id} className="procedureItem"><Tag color="blue">{index + 1}</Tag><Typography.Text strong>{procedure.name}</Typography.Text></div>)}</Flex>
        {operation.diagnosis && <div className="diagnosisBlock"><Typography.Text type="secondary">Diagnosis</Typography.Text><Typography.Paragraph style={{ margin: '4px 0 0' }}>{operation.diagnosis}</Typography.Paragraph></div>}
      </Card>
    </Flex>
  </Flex>;

  const team = <Flex vertical gap={16}>
    <Card title={<Flex align="center" gap={8}><TeamOutlined /> Doctors</Flex>}><div className="medicalTeamGrid">{doctors.length ? doctors.map((doctor) => <StaffCard key={doctor.id} name={doctor.name} role="Doctor" subtitle={(doctor.specialties ?? []).map((s) => s.name).join(', ')} phone={getPhone(doctor)} />) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} />}</div></Card>
    <Card title={<Flex align="center" gap={8}><TeamOutlined /> Nursing</Flex>}><div className="medicalTeamGrid">{nurses.length ? nurses.map((nurse) => <StaffCard key={nurse.id} name={nurse.name} role="Nurse" phone={getPhone(nurse)} />) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} />}</div></Card>
  </Flex>;

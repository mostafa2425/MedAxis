...
import { formatOperationDate, formatTime, getInitials, getSpecialtyLabel, getStatusColor } from '@/utils/helpers';
import { OPERATION_STATUSES } from '@/utils/constants';
import { OperationStatus, type Operation, type Doctor, type Nurse } from '@/types';
import { getApiErrorMessage } from '@/utils/apiValidationErrors';
import { getTimelineActionLabel } from '@/utils/timelineActions';
...
export default function OperationDetailPage() {
  ...
  const uploadMutation = useMutation({ mutationFn: ({ file, fileType }: { file: File; fileType: string }) => operationService.uploadFiles(id!, file, fileType), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['operation-detail', id] }); messageApi.success(i18n.language.startsWith('ar') ? 'تم رفع الملف بنجاح' : 'File uploaded successfully'); }, onError: (e) => messageApi.error(getApiErrorMessage(e, 'Unable to upload file')) });
  ...
  const upload = (file: File, fileType: string) => { uploadMutation.mutate({ file, fileType }); };

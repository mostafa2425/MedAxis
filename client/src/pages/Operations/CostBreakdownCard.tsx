...
const MoneyInput = ({ currency }: { currency: string }) => <Space.Compact style={{ width: '100%' }}><InputNumber min={0} precision={2} style={{ width: '100%' }} /><Typography.Text style={{ padding: '4px 10px', border: '1px solid #d9d9d9', display: 'flex', alignItems: 'center', background: '#fafafa' }}>{currency}</Typography.Text></Space.Compact>;
...
export default function CostBreakdownCard({ operationId, cost, currency = 'EGP' }: Props) {
  ...
  return <>{holder}<Card className="cost-card" title={<Flex align="center" gap={10}><DollarOutlined /><span>Financials</span></Flex>}>
    <Form form={form} layout="vertical" onFinish={(data) => mutation.mutate(data)}>
      <Flex gap={16} wrap>
        <Form.Item name="totalCost" label="Total Operation Cost" rules={[{ required: true, message: 'Enter total cost' }]} style={{ flex: '1 1 260px' }}><MoneyInput currency={currency} /></Form.Item>
      ...
      <Collapse ghost items={[{ key: 'breakdown', label: <Flex justify="space-between" align="center" style={{ width: '100%' }}><span>Cost Breakdown <Typography.Text type="secondary">(optional)</Typography.Text></span><Typography.Text>{money(expenses)} {currency}</Typography.Text></Flex>, children: <>
        ...
        <Flex gap={12} wrap>{[['hospitalCost', 'Hospital'], ['nursingCost', 'Nursing'], ['assistantDoctorsCost', 'Assistant Doctors'], ['equipmentCost', 'Equipment'], ['otherCost', 'Other']].map(([name, label]) => <Form.Item key={name} name={name} label={label} style={{ flex: '1 1 180px' }}><MoneyInput currency={currency} /></Form.Item>)}</Flex>
        {expenses > total && <Alert type="error" showIcon message="Breakdown exceeds total operation cost" />}
        <Divider />
        <Flex justify="space-between"><Typography.Text strong>Total Expenses</Typography.Text><Typography.Text strong>{money(expenses)} {currency}</Typography.Text></Flex>
        <Flex justify="space-between" style={{ marginTop: 8 }}><Typography.Text strong>Doctor Share</Typography.Text><Typography.Text strong>{money(doctorShare)} {currency}</Typography.Text></Flex>
      </> }]} />

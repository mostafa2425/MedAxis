...
export default function ProfilePage() {
  ...
  return (
    <div className="profile-page page">
      ...
      <Card className="profileCard"><h2 className="profileSectionTitle">{t('profile.accountInformation')}</h2><div className="profileReadonly"><div className="profileReadonlyItem"><span className="profileTagLabel">{t('common.email')}</span><span className="profileReadonlyValue"><MailOutlined /> {user.email}</span></div><div className="profileReadonlyItem"><span className="profileTagLabel">{t('profile.doctorId')}</span><span className="profileReadonlyValue"><IdcardOutlined /> {user.doctorId || '—'}</span></div></div><h2 className="profileSectionTitle">{t('profile.professionalInformation')}</h2><div className="profileTags"><span className="profileTagLabel">{t('profile.professionalSpecialties')}</span>{currentSpecialties.length > 0 ? currentSpecialties.map((specialty) => <Tag key={specialty.id}>{getSpecialtyLabel(specialty, i18n.language)}</Tag>) : <span className="profileReadonlyValue">{t('profile.noSpecialties')}</span>}</div><div className="profileTags"><span className="profileTagLabel">{t('profile.areasOfExpertise')}</span>{currentAreas.length > 0 ? currentAreas.map((specialty) => <Tag key={specialty.id} color="blue">{getSpecialtyLabel(specialty, i18n.language)}</Tag>) : <span className="profileReadonlyValue">{t('specialties.noAreasForSpecialty')}</span>}</div><Form form={form} layout="vertical" className="entityForm" onFinish={onFinish} requiredMark><Form.Item name="name" label={t('auth.fullName')} rules={[{ required: true, message: t('validation.required') }]}><Input prefix={<UserOutlined />} size="large" /></Form.Item><Form.Item name="phone" label={t('common.phone')}><Input prefix={<PhoneOutlined />} size="large" inputMode="tel" allowClear /></Form.Item><SpecialtyFields specialties={specialties} loading={specialtiesLoading} specialtyLabel={t('profile.professionalSpecialties')} specialtyPlaceholder={t('profile.selectSpecialties')} subspecialtyLabel={t('profile.areasOfExpertise')} subspecialtyPlaceholder={t('profile.selectAreas')} /><Form.Item className="entityFormActions"><Space><Button type="primary" htmlType="submit" loading={updateMutation.isPending}>{t('common.save')}</Button></Space></Form.Item></Form></Card>
    </div>
  );
}

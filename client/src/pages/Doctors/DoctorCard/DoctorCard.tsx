...
export default function DoctorCard({ doctor, onEdit, onDelete }: DoctorCardProps) {
  ...
  return (
    <Card className="doctorCard" size="small">
      ...
      {primarySpecialty && (
        <p className="doctorCardSubtitle">
          {getSpecialtyLabel(primarySpecialty, i18n.language)}
        </p>
      )}
      ...
      {(specialties.length > 0 || (doctor.subspecialties ?? []).length > 0) && (
        <div className="doctorCardTags">
          {specialties.map((specialty) => (
            <Tag key={specialty.id} className="doctorCardTag">
              {getSpecialtyLabel(specialty, i18n.language)}
            </Tag>
          ))}
          {(doctor.subspecialties ?? []).map((area) => (
            <Tag key={area.id} className="doctorCardTag" color="blue">

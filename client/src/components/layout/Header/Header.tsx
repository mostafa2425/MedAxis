...
function getPageMeta(pathname: string): { titleKey: string; hintKey?: string } {
  ...
  if (basePath === "/operations" && segments[2] === "edit")
    return {
      titleKey: "operations.editOperation",
      hintKey: "operations.title",
    };
  if (basePath === "/operations" && segments[1] && segments[1] !== "new")
    return {
      titleKey: "operations.operationDetails",
      hintKey: "operations.title",
    };
...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  const loadNotifications = async () => {
    try {
      setNotifications(await notificationService.list(20));
    } catch {
  ...
  const notificationDropdown = (
    ...
    >
      <div className="notificationDropdownHeader">
        ...
        {unreadCount > 0 && (
          <span className="notificationUnreadPill">{unreadCount}</span>
        )}
      </div>
      ...
      <div className="notificationList">
        {notifications.length === 0 ? (
          <div className="notificationEmpty">
            <span className="notificationEmptyIcon">
              <CheckOutlined />
            </span>
            <strong>
              {language === "ar"
            ...
            <span>
              {language === "ar"
                ? "سنعرض هنا أهم التنبيهات والموجزات."
                : "Important alerts and smart briefs will appear here."}
            </span>
          </div>
        ) : (
          notifications.slice(0, 8).map((item) => (
            ...
            >
                ...
                <span className="notificationItemTopline">
                  ...
                  <span className="notificationItemTime">
                    {formatNotificationTime(
                      item.created_at || item.scheduled_for,
                      language,
                    )}
                  </span>
                </span>
              ...
              {!item.read_at && (
                <span className="notificationUnreadDot" aria-label="Unread" />
              )}
            </button>
          ))
        )}
      ...
      <div className="notificationDropdownFooter">
        <button
          type="button"
          className="notificationFooterButton"
          onClick={() => {
            navigate("/assistant");
            setNotificationOpen(false);
          }}

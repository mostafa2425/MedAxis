...
interface Step { key: string; title: string; description: string; route: string; icon: ReactNode; }
...
export default function PracticeSetupTour({ patients, hospitals, doctors }: Props) {
  ...
  useEffect(() => { if (steps.length && localStorage.getItem(DISMISS_KEY) !== '1') { const timer = window.setTimeout(() => setOpen(true), 700); return () => window.clearTimeout(timer); } return undefined; }, [steps.length]);
  ...
  return <Modal open={open} onCancel={close} footer={null} centered width={440} className="practiceSetupTour"><div className="practiceSetupTour__icon">{current.icon}</div><Tag color="blue">Setup {index + 1} of {steps.length}</Tag><h2>{current.title}</h2><p>{current.description}</p><Progress percent={Math.round(((index + 1) / steps.length) * 100)} showInfo={false} size="small" /><div className="practiceSetupTour__actions"><Button onClick={close}>Later</Button><Button type="primary" icon={index === steps.length - 1 ? <CheckCircleOutlined /> : <RightOutlined />} onClick={go}>{index === steps.length - 1 ? 'Start setup' : 'Go there'}</Button>{index < steps.length - 1 && <Button type="link" onClick={next}>Next</Button>}</div></Modal>;
}
